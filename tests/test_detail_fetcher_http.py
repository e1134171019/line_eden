...
def _fetcher(max_download_bytes: int = 1024 * 1024) -> AnnouncementDetailFetcher:
    return AnnouncementDetailFetcher(10.0, "ScholarshipAgentTest/1.0", 3, max_download_bytes, 10)
...
def test_fetch_html_and_attachment_evidence(monkeypatch: pytest.MonkeyPatch) -> None:
    html = """
    <div class="mpgdetail"><h2>能源獎學金</h2>
    <p>申請資格請參閱附件。</p><a href="/files/rules.pdf">評選辦法.pdf</a></div>
    """
...
def test_short_url_redirects_to_pdf(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _fetcher().fetch_text(_item("https://reurl.cc/example")) == "大專學生可申請。"
