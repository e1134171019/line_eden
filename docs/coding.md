# 程式設計規範

## 分層

```text
Collector
→ Detail Fetcher
→ Model / Normalizer
→ Profile Loader
→ Eligibility Evaluator
→ Repository
→ Formatter
→ Notifier
```

## 核心原則

- 公告列表抓取、公告內頁讀取、資格判斷、SQLite 與通知傳送必須分層。
- 確定性狀態由 Python 與 SQLite 負責。
- `eligible` 才進入預設推播清單。
- `review` 與 `ineligible` 保留判斷狀態，但預設不推播。
- 通知管道必須實作 `NotificationChannel`，服務層不得依賴 LINE 或 Apprise 細節。
- 每個啟用管道成功後分別寫入 delivery ledger；全部成功後才能寫入 `notified_at`。
- 通知內容由 Jinja2 模板渲染，模板不得讀取憑證或學生私密背景。
- 背景設定變更時，使用背景指紋重新評估尚未通知公告。
- 公告 identity 只使用來源與正規化網址；listing metadata 不得作為穩定 ID。
- `revision_hash` 只反映正文、附件與辦法狀態；抽取 policy hash 必須分開保存。
- 來源專屬正文範圍以版本化 policy 設定，找不到 strict selector 時採失敗關閉。
- 外部 HTTP 請求必須設定 timeout 並明確處理錯誤。
- LINE 憑證與 Apprise URL 放在 `.env`。
- 學生背景放在 `profile.json`。
- `.env` 與 `profile.json` 都不得提交到 GitHub 或輸出至 log。
