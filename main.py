...
from src.formatters.cli_formatter import print_audit, print_items, print_summary
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    modes.add_argument("--audit", action="store_true", help="重新稽核全部公告但不改狀態")
    modes.add_argument(
        "--initialize-baseline",
        action="store_true",
...
def parse_mode(args: argparse.Namespace) -> RunMode:
...
def build_live_notifier() -> Notifier:
...
def _build_collector() -> LhuCollector:
    return LhuCollector(LHU_SCHOLARSHIP_URL, HTTP_TIMEOUT_SECONDS, HTTP_USER_AGENT)
...
def build_full_service(mode: RunMode, use_gemini: bool = False) -> ScholarshipService:
...
def build_baseline_service() -> ScholarshipService:
    return ScholarshipService(
        _build_collector(),
        _build_repository(),
        build_notifier(RunMode.INITIALIZE_BASELINE),
        include_keywords=SCHOLARSHIP_FILTER_KEYWORDS,
        summary_batch_size=LINE_SUMMARY_BATCH_SIZE,
    )
...
def build_service(
    ...
    """相容舊呼叫；新程式應使用明確的 full/baseline factory。"""
    if not profile_required:
        return build_baseline_service()
    return build_full_service(mode, use_gemini)
...
def execute_service(
    ...
    return service.run(dry_run=mode is RunMode.DRY_RUN)
...
def main(argv: list[str] | None = None) -> None:
    ...
    service = (
        ...
        if mode is RunMode.INITIALIZE_BASELINE
        else build_full_service(mode, use_gemini=args.use_gemini)
    )
