...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-gemini",
        action="store_true",
        help="啟用掃描 PDF 備援；audit 同時執行文字 structured shadow",
    )
...
def _print_audit_record(record: AuditRecord) -> None:
    ...
    comparison = record.structured_comparison
    if comparison:
        print(
            "  structured="
            f"{comparison.structured_status} | changed={comparison.changed} | "
            f"{comparison.structured_reason}"
        )
