...
class LhuCollector(BaseCollector):
    ...
    # 使用共用 paginator 抓取入口頁或全部分頁，再套用龍華表格解析規則。
    def _collect_lhu(self) -> list[Scholarship]:
        ...
        records, raw_rows = self._parse_listing_pages(result.pages)
        ...
        if not records and result.errors:
            raise RuntimeError(error)
        return records

    # 合併所有成功頁面的公告，維持以公告 URL 去重的既有行為。
