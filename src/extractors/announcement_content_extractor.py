...
def select_announcement_root(
    ...
    policy = policy_for_url(source_url)
    _remove_boilerplate(soup, policy)
    selected, _, _ = _select_with_policy(soup, title, policy)
    return selected
...
def _select_with_policy(
    ...
    for selector in policy.include_selectors:
        ...
        selected = _smallest_reliable_candidate(
            candidates,
            title,
    ...
    selected = max(_unique_tags(candidates), key=_content_score, default=None)
    return selected, "heuristic", True
...
def _remove_boilerplate(soup: BeautifulSoup, policy: DetailExtractionPolicy) -> None:
    selector = ", ".join(policy.subtractive_selectors)
