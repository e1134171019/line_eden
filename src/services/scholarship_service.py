...
from src.services.gemini_shadow_extraction_service import analyze_text_shadow
...
@dataclass(frozen=True)
class AuditRecord:
    """單筆歷史公告的 legacy 與 structured shadow 稽核結果。"""

    item: Scholarship
    detail_excerpt: str
    fetch_result: DetailFetchResult
    gemini_diagnostic: GeminiAnalysisDiagnostic | None = None
    structured_decision: EligibilityDecision | None = None
    structured_checks: tuple[ConditionCheck, ...] = tuple()
...
class ScholarshipService:
    ...
    def __init__(
        ...
        self.structured_evaluator = StructuredEligibilityEvaluator()
    ...
    def audit(self) -> AuditResult:
        ...
        differences = self._count_structured_differences(records)
        message = f"已稽核 {len(records)} 筆公告，不會傳送 LINE 或修改獎學金狀態。"
        if self.gemini_fallback:
            message += " structured shadow 只寫入獨立 Gemini 快取，不影響正式推播。"
        ...
        return AuditResult(
            ...
            structured_difference_count=differences,
        )
    ...
    def _build_audit_record(self, item: Scholarship) -> AuditRecord:
        ...
        structured_checks: tuple[ConditionCheck, ...] = tuple()
        structured_diagnostic: GeminiAnalysisDiagnostic | None = None
        if notice_kind == APPLICATION and self.gemini_fallback and gemini is None:
            shadow = analyze_text_shadow(self.gemini_fallback, item.title, fetch_result)
            if shadow is not None:
                structured_diagnostic = shadow.diagnostic
                if shadow.extraction is not None:
                    structured = self.structured_evaluator.evaluate(shadow.extraction, self.profile)
                    structured_decision = structured.decision
                    structured_checks = structured.checks
        evaluated = replace(
            item,
    ...
    def _count_structured_status(self, records: list[AuditRecord], status: str) -> int:
        return sum(
            record.structured_decision is not None
            and record.structured_decision.status == status
            for record in records
        )

    def _count_structured_differences(self, records: list[AuditRecord]) -> int:
        return sum(
            record.structured_decision is not None
            and record.structured_decision.status != record.item.eligibility_status
            for record in records
        )
