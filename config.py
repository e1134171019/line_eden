...
def _env_bool(name: str, default: bool) -> bool:
    ...
    if raw_value in {"0", "false", "no", "off"}:
        return False
    raise RuntimeError(f"環境變數 {name} 必須是 true 或 false")
...
# 僅保留給舊快取／舊測試資料相容；新擷取流程應使用 DetailFetchResult.rules_status。
