...
EXPECTED_PROGRAM_WATCH_COUNT = 30


def main() -> None:
    """完整抓取六個核心來源及 30 方案監測群組，只輸出來源診斷。"""
    print(
    ...
    watch = diagnostics[-1]
    if watch.source != "TUN 30方案官方監測":
        raise SystemExit("第七個來源群組不是 TUN 30方案官方監測。")
    if watch.status != "success" or watch.completeness != "complete":
        raise SystemExit(f"30 方案分頁尚未完整：{watch.error or watch.stop_reason}")
    if watch.child_sources_detected != EXPECTED_PROGRAM_WATCH_COUNT:
        raise SystemExit("30 方案監測目錄數量不正確。")
    if watch.child_sources_succeeded != EXPECTED_PROGRAM_WATCH_COUNT:
        raise SystemExit("30 方案仍有入口無法連線。")
