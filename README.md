...
每個來源由專用 Collector 限定入口與解析規則；服務層不再使用全域標題關鍵字刪除公告。因此「鴻海獎學鯨」等非標準方案名稱仍會進入正文解析、公告分類與資格判斷。
...
TUN 只用來發現方案名稱，不作為資格與期限證據。每個方案入口明確分成：

- `listing`：公告列表會持續新增項目，完整稽核追蹤分頁，每日增量補抓前兩頁。
- `fixed_page`：辦法頁以內容修訂代表更新，只抓固定頁，不誤追網站導覽分頁。
- `dynamic_page`：保留官方動態入口；靜態回應未命中方案時失敗關閉。
- `core_covered`：沒有穩定主辦單位入口時，由教育部圓夢助學網核心來源探索。

完整稽核的方案成功必須同時符合「入口可取得、方案別名命中、成功建立公告」；
只有 HTTP 200 不會再被計為方案成功。去重鍵包含申請年度，同名不同年度會保留兩筆。
七階段與驗收情境詳見 `docs/source-monitoring.md`。
...
- ODT 文字內容。
- 公告詳情到附件中介頁的兩層追蹤。
- 公開 Google Drive／Docs 檢視網址轉換為下載入口。
- 具有 `data-url` 或簡單 `window.open(...)` 的下載按鈕。
